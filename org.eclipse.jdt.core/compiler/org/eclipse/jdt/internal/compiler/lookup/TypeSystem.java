...
//{ObjectTeams: capability to store more properties:
/* orig:
		private final class InternalParameterizedTypeBinding extends ParameterizedTypeBinding {
  :giro */
		private final class InternalParameterizedTypeBinding extends DependentTypeBinding {
	...
	/*
				this(genericType, typeArguments, enclosingType, environment);
	  :giro */
				this(genericType, typeArguments, null, -1, enclosingType, environment);
// SH}
			}
			public InternalParameterizedTypeBinding(ReferenceBinding genericType, TypeBinding[] typeArguments, ITeamAnchor teamAnchor, int valueParamPosition,
				...
				super(genericType, typeArguments, teamAnchor, valueParamPosition, enclosingType, environment);
			}
...
//{ObjectTeams: more to check:
						&& isRoleTypeMatch(this._teamAnchor, this._valueParamPosition, that);
// SH}
			}
...
//{ObjectTeams: more to encode:
				if (this._teamAnchor != null)
					hashCode += 3 * CharOperation.hashCode(this._teamAnchor.getBestName());
				hashCode += 5 * this._valueParamPosition;
// SH}
				return hashCode;
			}
		}
...
/* orig
  ...
  :giro */
		ParameterizedTypeBinding get(ReferenceBinding genericType, TypeBinding[] typeArguments, ITeamAnchor teamAnchor, int valueParamPosition,
...
//{ObjectTeams: more checks:
				  if (isRoleTypeMatch(teamAnchor, valueParamPosition, parameterizedType))
// SH}
